...
class ArchimedDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some archimedd methods
    ...
    @classmethod
    def from_archimed_conf(self, archimed_dot_conf):
        from archimed_config import ArchimedConfig
        config_text = ArchimedConfig.slurp_config_file(archimed_dot_conf)
        creds = ArchimedConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = archimedlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = archimedlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = archimedlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return False # (getinfo['protocolversion'] >= config.min_archimedd_proto_version_with_sentinel_ping)
