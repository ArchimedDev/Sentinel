...
def is_valid_archimed_address(address, network='mainnet'):
    ...
    archimed_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Archimed address
        return False

    if (address_version != archimed_version):
        return False
...
# shims 'til we can fix the archimedd side
def SHIM_serialise_for_archimedd(sentinel_hex):
    ...
    # superblock => "trigger" in archimedd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # archimedd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    archimedd_hex = serialise(obj)
    return archimedd_hex


# shims 'til we can fix the archimedd side
def SHIM_deserialise_from_archimedd(archimedd_hex):
    from models import COIN2FLYD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(archimedd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('archimed.conf')
