...
sentinel_cfg = ArchimedConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_archimedd_proto_version_with_sentinel_ping = 70207


def get_archimed_conf():
    home = os.environ.get('HOME')

    archimed_conf = os.path.join(home, ".archimedcore/archimed.conf")
    if sys.platform == 'darwin':
        archimed_conf = os.path.join(home, "Library/Application Support/ArchimedCore/archimed.conf")

    archimed_conf = sentinel_cfg.get('archimed_conf', archimed_conf)

    return archimed_conf
...
archimed_conf = get_archimed_conf()
network = get_network()
db = get_db_conn()
