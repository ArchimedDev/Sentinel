...
class GovernanceClass(object):
    ...
    def vote(self, archimedd, signal, outcome):
        return self.go.vote(archimedd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, archimedd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(archimedd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(archimedd, models.VoteSignals.valid, models.VoteOutcomes.no)
    ...
    def list(self):
        ...
        # return a dict similar to archimedd "gobject list" output
    ...
    def archimedd_serialise(self):
        import archimedlib
        return archimedlib.SHIM_serialise_for_archimedd(self.serialise())
