...
def has_archimed_conf():
    import config
    import io

    valid_archimed_conf = False

    # ensure archimed_conf exists & readable
    #
    # if not, print a message stating that Archimed Core must be installed and
    # configured, including JSONRPC access in archimed.conf
    try:
        f = io.open(config.archimed_conf)
        valid_archimed_conf = True
    except IOError as e:
        print(e)

    return valid_archimed_conf
...
def main():
    ...
    if not has_archimed_conf():
        print("ArchimedCore must be installed and configured, including JSONRPC access in archimed.conf")
        sys.exit(1)
