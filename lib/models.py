...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, archimedd):
        golist = archimedd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_archimedd(archimedd, item)
    ...
    @classmethod
    def import_gobject_from_archimedd(self, archimedd, rec):
        import decimal
        import archimedlib
        import inflection
        ...
        # shim/archimedd conversion
        object_hex = archimedlib.SHIM_deserialise_from_archimedd(object_hex)
        objects = archimedlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from archimedd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from archimedd, with every run
        ...
        # get/create, then sync payment amounts, etc. from archimedd - Archimedd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(archimedd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from archimedd! %s" % e)
            govobj.vote_delete(archimedd)
            return (govobj, None)
    ...
    def vote_delete(self, archimedd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(archimedd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, archimedd, signal, outcome):
        import archimedlib
        ...
        output = archimedd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = archimedlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(archimedd, signal)

    def sync_network_vote(self, archimedd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = archimedd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 archimed addr, non-multisig
            if not archimedlib.is_valid_archimed_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Archimed address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (archimedlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/ArchimedDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import archimedlib
        obj_data = archimedlib.SHIM_serialise_for_archimedd(self.serialise())
    ...
    def prepare(self, archimedd):
        try:
            object_hash = archimedd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not archimedlib.is_valid_archimed_address(addr, config.network):
    ...
    def hash(self):
        import archimedlib
        return archimedlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, archimedd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - archimedd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, archimedd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - archimedd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, archimedd):
        now = int(time.time())
        return (self.created_at < (now - archimedd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, archimedd):
        if self.is_expired(archimedd):
            return False

        return True

    def is_deletable(self, archimedd):
        if self.is_expired(archimedd):
            return True
